...
// Singleton guard to avoid double-initialization
if (window.__VatexBridgeInstalled) {
  console.warn(
    "Vatex content bridge already installed - skipping duplicate init"
  );
} else {
  ...
  fetch(chrome.runtime.getURL("index.html"))
    ...
    .then((html) => {
       ...
       */
      if (!window.__VatexPageBridgeListenerAdded) {
        ...
        window.__VatexPageBridgeListenerAdded = true;
      }
  ...
  if (!window.__VatexPageToBgListenerAdded) {
    window.addEventListener("message", function (event) {
      if (event.source !== window) return;
      ...
      if (event.data.type === "FROM_PAGE_TRANSCRIBE_AUDIO") {
        const msg = event.data.message || {};
        const requestId =
          msg.requestId ||
          `tx-${Date.now()}-${Math.random().toString(36).slice(2)}`;
        chrome.runtime.sendMessage({ ...msg, requestId }, (response) => {
          // Forward transcription response back to the page with requestId
          window.postMessage(
            {
              type: "FROM_EXTENSION_TRANSCRIBE_AUDIO",
              requestId,
              response: response,
            },
            "*"
    ...
    window.__VatexPageToBgListenerAdded = true;
  }
} // end singleton guard
